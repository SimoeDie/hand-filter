...
//
//  GESTI:
//   - Pollice + Indice di entrambe le mani  -> crea il rettangolo "finestra"
...
function draw() {
    ...
    if (windowActive && windowAlpha > 0.01) {
        ...
        // Ritaglia con la finestra (rettangolo) e lo usa come maschera luminosa
        blendMode(BLEND);
        // Taglia l'area del rettangolo
        let rect = currentRect;
        if (rect) {
            // Salvo il contenuto attuale, disegno l'effetto sopra
            tint(255, windowAlpha * 255);
            drawingContext.save();
            drawingContext.beginPath();
            drawingContext.rect(rect.x1, rect.y1, rect.x2 - rect.x1, rect.y2 - rect.y1);
            drawingContext.clip();
...
function handleGestures() {
    ...
    // GESTO 1: Pollice+Indice di entrambe le mani -> rettangolo
    // Calcolo il rettangolo che ha come vertici i 4 punti mano-pollice/indice
...
// Schermata-style ritaglio: in realtà l'effetto è applicato solo dentro il rettangolo
// tramite il clip nel draw(). Qui le funzioni applicano l'effetto a tutto il buffer
// e il clip nel draw() si occupa di mostrarlo solo dentro il rettangolo.

function applySepiaEffect() {
    // Effetto seppia (toni caldi vintage) calcolato a mano sui pixel
    ...
    for (let i = 0; i < p.length; i += 4) {
        ...
        let g = p[i + 1];
        let b = p[i + 2];
        p[i]     = min(255, 0.393 * r + 0.769 * g + 0.189 * b);
        p[i + 1] = min(255, 0.349 * r + 0.686 * g + 0.168 * b);
        p[i + 2] = min(255, 0.272 * r + 0.534 * g + 0.131 * b);
    }
...
function applyPixelEffect() {
    ...
    filterBuffer.loadPixels();
    let cols = filterObjs.pixel.cols;
    let rows = filterObjs.pixel.rows;
    // Rendo il buffer pixelato campionando blocchi
    for (let i = 0; i < cols; i++) {
        for (let j = 0; j < rows; j++) {
            let x = i * size;
            let y = j * size;
            // Leggo il colore medio del blocco
            let r = 0, g = 0, b = 0, count = 0;
            for (let dx = 0; dx < size; dx += 3) {
                for (let dy = 0; dy < size; dy += 3) {
                    let idx = ((y + dy) * width + (x + dx)) * 4;
                    r += filterBuffer.pixels[idx];
                    g += filterBuffer.pixels[idx + 1];
                    b += filterBuffer.pixels[idx + 2];
                    count++;
                }
            }
            r /= count; g /= count; b /= count;
            filterBuffer.noStroke();
            filterBuffer.fill(r, g, b);
...
function applyGlitchEffect() {
    // Scorre il buffer e disloca casualmente alcune fasce orizzontali
    let y = (frameCount * 7) % height;
    let band = 5 + Math.floor(Math.random() * 20);
    let shiftAmt = (Math.random() * 30) - 15;
    // Copio la fascia da dislocare
    let slice = filterBuffer.get(0, y, width, band);
    if (slice) {
        filterBuffer.image(slice, shiftAmt, y);
    }
}

function applyPsychedelicEffect() {
    // Shift dei canali colore per effetto psichedelico
    filterBuffer.loadPixels();
    let p = filterBuffer.pixels;
...
function applyWaveEffect() {
    // Distorsione ondulata del video
    filterObjs.wave.time += 0.05;
    let strength = 15;
    let cols = Math.floor(width / 4);
    for (let i = 0; i <= cols; i++) {
        let offset = Math.sin(i * 0.35 + filterObjs.wave.time) * strength;
        let slice = filterBuffer.get(i * 4, 0, 4, height);
        if (slice) {
            filterBuffer.image(slice, i * 4 + offset, 0);
        }
    }
}
...
// ---------------- CORNICE DECORATIVA ----------------
function drawDecorativeFrame(rect) {
    ...
    let pulse = 1 + Math.sin(frameTimer * 0.1) * 0.1;
    rect(rect.x1, rect.y1,
        (rect.x2 - rect.x1) * pulse,
        (rect.y2 - rect.y1) * pulse,
        10);
    // Piccoli elementi decorativi sugli angoli
